...
client.eth.getAccounts().then(async function (accounts) {
    ...
    // get ether from bank.
    await bank.methods.getBankBalance()
        .call({ from: accounts[0], gas: 0, gasPrice: 0, value: 0 })
        .then(function (balance) { console.log("Contract address:\t", cntrtAdr, "\nNow address:\t\t", accounts[0], "\nBalance in bank (wei):\t", balance) })
