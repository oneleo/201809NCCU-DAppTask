...
client2.eth.getAccounts().then(async function (accounts) {
    ...
    let bank2 = await new client2.eth.Contract(abi, cntrtAdr);
    ...
    client1.eth.getAccounts().then(async function (accounts) {
        ...
        // your code
        bank2.methods.transferOwner(accounts[0])
            .send({ from: address2, gas: gasLmt, gasPrice: gasPrc, value: 0 })
            .then(function (receipt) { console.log("Contract address:\t", cntrtAdr, "\nOriginal owner:\t\t", address2, "\nNew owner:\t\t", accounts[0], "\nTransfer owner info:\n", receipt) })
